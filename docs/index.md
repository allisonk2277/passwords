...
The common theme with these schemes is that they may be clever, and a newbie attacker with no experience might have a hard time figuring out what you did. However, our threat model is not that of a newbie attacker; we wish to defend against highly experienced attackers. In our threat model, we assumed that the attacker *knows the procedure you used*; the examples above rely on the attacker not knowing this information and lose nearly all of their value if that ceases to be true. If you're not convinced that your password scheme is weak, consider these questions:
...
* Perhaps most importantly, how many possible passwords can you really have generated with one of these methods? Does it come anywhere close to the 80 bits of entropy we've been aiming for (2^80 = 1.2e24 = 1.2 trillion trillion possibilities)? If you can only generate 1,000 or even 1,000,000 possibilities, then it is broken.
...
2. Each time you create an account with a website, generate a *unique* strong password and store it using the password manager. You should never have a reason to use the same password on more than one website. Password reuse is one of the most common methods by which accounts are hacked - attackers will compromise a random site (like an old web forum running phpBB from 2006), then turn around and try all the learned username/password combinations on more valuable sites like your email and bank accounts. See also: [https://xkcd.com/792/](https://xkcd.com/792/).
3. Do use a 2-factor (2FA) method to further protect your accounts. Ideally a token- or hardware device-based mechanism like [Google Authenticator](https://en.wikipedia.org/wiki/Google_Authenticator) or [YubiKey](https://en.wikipedia.org/wiki/YubiKey) is best, but if those aren't available then text-message based 2FA is okay too.
  * Text-message based 2FA is inherently weaker because it relies on the security of the cellular networks in order to be secure; see [this article](https://www.theverge.com/2017/9/18/16328172/sms-two-factor-authentication-hack-password-bitcoin). Token based 2FA like Google Authenticator is more secure because there is no communication from the website to your phone for an attacker to intercept. Do note that you're still vulnerable to phishing if you're manually entering the token into a website - a YubiKey [defends against this](https://www.yubico.com/phishing/) by taking an additional step of cryptographically checking the website first.
...
It's worth mentioning that writing down your password could be a good idea so that friends and/or family members may have access to important documents/etc should you have a medical emergency. If you're really into cryptography and have trusted friends who are as well, you could also consider encrypting a file containing your master password with two (or more) of your friends' public keys, such that no single person could unilaterally decrypt your data.
